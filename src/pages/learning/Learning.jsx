...
const StatusBadge = ({ status }) => {
  const map = {
    interested:  { label: "Interested",  cls: "bg-blue-50 text-blue-600 border-blue-100" },
      ...
      <div className="bg-white w-full sm:max-w-lg rounded-t-3xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[85vh]">
        ...
        <div className="px-5 py-3 shrink-0">
          <div className="relative">
            ...
            <input
              ...
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search..."
              className="w-full pl-8 pr-4 py-2 text-sm border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>
          <div className="flex items-center justify-between mt-2.5">
            <span className="text-xs text-gray-400">{filtered.length} item{filtered.length !== 1 ? "s" : ""}</span>
              ...
              {filtered.map((item) => {
                ...
                return (
                  <div
                    ...
                    onClick={() => toggle(item.id)}
                    className={`flex items-start gap-3 rounded-xl px-3 py-3 cursor-pointer border transition-all ${
                      isChecked ? "border-blue-300 bg-blue-50" : "border-gray-100 hover:border-gray-200 bg-white"
                    }`}
                  >
                    <div className="mt-0.5 shrink-0">
                      {isChecked
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <p className={`text-sm font-semibold ${isChecked ? "text-blue-900" : "text-gray-900"}`}>
                          {item.title}
                        </p>
                        {item.category && (
                          <span className="text-[10px] bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">
                      ...
                      {resources.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mt-1.5" onClick={(e) => e.stopPropagation()}>
                          {resources.map((r) => (
                            <a key={r.id} href={r.url} target="_blank" rel="noopener noreferrer"
                              className="inline-flex items-center gap-1 px-2 py-0.5 bg-gray-50 text-gray-500 border border-gray-200 rounded-full text-[10px] font-medium hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 transition"
                            >
                              {resourceIcon(r.resource_type)} {r.title}
                            </a>
                          ))}
        ...
        <div className="px-5 py-4 border-t border-gray-100 shrink-0">
          <button
            onClick={() => onSave(selected, onClose)}
            disabled={selected.size === 0 || saving}
            className="w-full flex items-center justify-center gap-2 py-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed text-white text-sm font-semibold rounded-xl transition-all"
          >
            {saving ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />}
            {saving
              ? "Saving..."
  ...
  return (
    <div className="max-w-4xl mx-auto px-3 sm:px-0 pb-10">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-4">
          <p className="text-2xl font-bold text-blue-600">{allLearning.length}</p>
          <p className="text-xs text-gray-400 mt-0.5">Total learning interests</p>
        </div>
        <div className="bg-white border border-gray-100 rounded-xl p-4 col-span-2 sm:col-span-1">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Top Topics</p>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {emp.items.map((item) => {
                          ...
                          return (
                            <div
                              key={item.service_id || item.id}
                              className="flex flex-col gap-1.5 bg-white border border-blue-100 rounded-lg px-3 py-2"
                            >
                              {/* Title + Status */}
                              <div className="flex items-center gap-1.5 flex-wrap">
                                ...
                                <div className="flex flex-wrap gap-1.5 pl-4">
                                  {itemResources.map((r) => (
                                    <a
                                      ...
                                      target="_blank"
                                      rel="noopener noreferrer"
                                      className="inline-flex items-center gap-1 px-2 py-0.5 bg-blue-50 text-blue-600 border border-blue-100 rounded-full text-[10px] font-medium hover:bg-blue-100 transition"
                                    >
                                      {resourceIcon(r.resource_type)} {r.title} <ExternalLink size={9} />
                                    </a>
                                  ))}
    ...
    <div className="max-w-3xl mx-auto px-3 sm:px-0 pb-10">
      ...
      <div className="flex items-start justify-between mb-6 gap-4">
        ...
        {unmarked.length > 0 && (
          <button
            onClick={() => setShowModal(true)}
            className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-xl transition-all shadow-sm shadow-blue-200 shrink-0"
          >
            <Plus size={15} /> Add
          </button>
        )}
      ...
      {myLearning.length === 0 ? (
        ...
        <>
          ...
          <div className="space-y-2">
            {filteredMy.map((item) => (
              <div key={item.service_id} className="bg-white border border-blue-100 rounded-xl px-4 py-3.5 flex items-start gap-3">
                <CheckCircle2 size={17} className="text-blue-400 mt-0.5 shrink-0" />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-sm font-semibold text-gray-900">{item.title}</p>
                    <StatusBadge status={item.status} />
