...
import { Link } from "react-router-dom";
import { useAuth } from "../../context/AuthContext";
import { serviceAPI, resourceAPI } from "../../services/api";
...
const SectionHeading = ({ title, count }) => (
  <div className="flex items-center gap-3 mb-4">
    ...
    <span className="px-2 py-0.5 bg-gray-100 text-gray-400 rounded-full text-xs font-semibold">{count}</span>
...
export default function Services() {
  ...
  // ─── Split: my vs others ───────────────────────────────────────────────────
  const myServices    = services.filter((s) => s.created_by === user?.uid);
  const otherServices = services.filter((s) => s.created_by !== user?.uid);
  ...
  const filteredMine   = filterList(myServices);
  const filteredOthers = filterList(otherServices);
    ...
    <div className="p-4 md:p-6 max-w-6xl mx-auto pb-24 md:pb-6">
      ...
      {showForm && (
        <div className="bg-white rounded-2xl border border-gray-200 p-5 mb-6 space-y-5">
          ...
          <div className="border-t border-gray-100 pt-4">
            <div className="flex items-center justify-between mb-3">
              <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">📎 Resources (optional)</h3>
              <button
                onClick={() => setShowResourceForm(!showResourceForm)}
                className="text-xs text-blue-600 font-medium hover:underline"
              >
            ...
            {showResourceForm && (
              <div className="mb-3">
                <InlineResourceInput onAdd={handleStagedResource} onCancel={() => setShowResourceForm(false)} />
              </div>
            )}
              ...
              <ul className="space-y-1.5">
                {pendingResources.map((r, i) => (
                  <li key={i} className="flex items-center justify-between px-3 py-2 bg-gray-50 rounded-lg text-sm">
                    ...
                    <button onClick={() => removePendingResource(i)} className="text-red-400 hover:text-red-600 text-xs ml-3">
                      ✕ Remove
                    </button>
                  </li>
                ))}
      ...
      {loading ? (
        ...
        <>
          ...
          {filteredMine.length > 0 && (
            <div className="mb-8">
              <SectionHeading title="My Services" count={filteredMine.length} />
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredMine.map((s) => (
                  <ServiceCard key={s.id} service={s} isOwner={true} />
                ))}
              </div>
            </div>
          )}
          ...
          {filteredOthers.length > 0 && (
            <div>
              <SectionHeading title="Other Services" count={filteredOthers.length} />
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {filteredOthers.map((s) => (
                  <ServiceCard key={s.id} service={s} isOwner={false} />
                ))}
              </div>
            </div>
          )}
...
function InlineResourceInput({ onAdd, onCancel }) {
  ...
  const [form, setForm] = useState({ title: "", url: "", description: "", resource_type: "video" });
  const [err, setErr]   = useState("");
  ...
  return (
    <div className="bg-gray-50 rounded-xl border border-gray-200 p-4 space-y-3">
      ...
      <div className="flex gap-2">
        <button onClick={handleAdd} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg transition">
          + Stage Resource
        </button>
        <button onClick={onCancel} className="px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-600 text-xs font-medium rounded-lg transition">
          Cancel
        </button>
      </div>
    </div>
